...
We welcome contributions to **PedNStream**, from a simple comment or a question, to a full fledged [pull request](https://help.github.com/articles/about-pull-requests/). 
...
- You want to update or documentation to PedNstream.

The figure below summarizes the workflow we follow in development. We encourage contributors to adopt it whenever possible. The sections below outline the steps to make a contribution for each of the aforementioned cases.

![pednstream development workflow]()

## A.  You have a question

1. Use the search functionality [here](https://github.com/WaimenMak/PedNStream/issues) to see if someone already filed the same issue.
...
PedNstream is been developed for **Python 3.11**, and tested on Windows, Linux, and MacOS. 
...
If you have write access to the PedNstream GitHub repository, skip to the section [Develop your contribution](CONTRIBUTING.md#develop-your-contribution). If you are a first-time contributor, follow the below steps:

1. Go to the [PedNstream GitHub repository](https://github.com/WaimenMak/PedNStream) and click on 'Fork'. This will create a copy of the PedNStream repository in your GitHub account. 
            
2. Clone the project to your local computer:
        
    ```bash
    git clone git@github.com:<your-username>/PedNStream.git
    ```
...
3. Set up a development environment on your computer by installing PedNStream in development mode with the following command: (Consider using a virtual environment for this purpose.)
...
1. Push your feature branch to (your fork of) the PedNStream GitHub repository.
...
- If your pull request completes an issue, you would want to link that pull request to the specific issue it addresses. By doing so, when your pull request is merged, the issue will be closed automatically. To do this, you need to use one of the following keywords in your pull request's description or in a commit message, followed by `#` and the issue number (e.g., `#42`):
  ...
  - `resolves`
  - `resolved`
- Note that **your pull request must be to the** `dev` branch. You can read more about it [here](https://docs.github.com/en/issues/tracking-your-work-with-issues/using-issues/linking-a-pull-request-to-an-issue) on GitHub Docs.

## D. You want to improve the documentation

We use Sphinx and Markdown to write documentation for the PedNStream. The root of the documentation is the `docs/` directory. For specific parts, ReStructuredText is also used.

1. [Announce your plan.](https://github.com/WaimenMak/PedNStream/blob/dev/CONTRIBUTING.md#announce-your-plan)
2. Follow the same steps to set up a development environment for [making changes to the code base](https://github.com/WaimenMak/PedNStream/blob/dev/CONTRIBUTING.md#set-up-a-local-development-environment-to-work-on-your-changes).
...
6. [Submit your contribution](https://github.com/WaimenMak/PedNStream/blob/dev/CONTRIBUTING.md#submitting-your-contribution) for review.
