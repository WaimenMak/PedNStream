...
class NetworkDashboard:
    ...
    def update_links(self, m, time_step, variable):
        ...
        colormap = LinearColormap(
            colors=colors, vmin=vmin, vmax=vmax, caption=variable.capitalize()
        )
        ...
        if format.lower() == "mp4":
            ...
            try:
                ...
                with imageio.get_writer(
                    output_path, mode="I", duration=1.0 / fps
                ) as writer:
                    for frame in frames:
                        writer.append_data(frame)
            except ImportError:
                st.warning(
                    "For better GIF quality, install imageio: pip install imageio"
                )
        ...
        try:
            for i, t in enumerate(range(start_time, end_time)):
                ...
                folium.plugins.FloatImage(
                    f"data:image/svg+xml;base64,{svg_encoded}", bottom=5, left=5
                ).add_to(m)
    ...
    def run_dashboard(self):
        ...
        with col1:
            # Time step slider
            time_step = st.slider("Time Step", 0, self.max_time - 1, value=0)
        ...
        with st.expander("Advanced Video Options"):
            ...
            with col6:
                video_width = st.number_input(
                    "Width (pixels)", min_value=800, max_value=1920, value=1200
                )
            with col7:
                video_height = st.number_input(
                    "Height (pixels)", min_value=600, max_value=1080, value=800
                )
                ...
                with st.spinner("Generating video... This may take a few minutes."):
                    try:
                        ...
                        if video_path and os.path.exists(video_path):
                            ...
                            st.info(
                                f"📊 Video info: {video_end - video_start + 1} frames, {fps} FPS, {file_size:.1f} MB"
                            )

                            # Display preview for small GIFs
                            if (
                                video_format == "gif" and file_size < 10
                            ):  # Only show GIFs smaller than 10MB
                                st.image(video_path, caption="Video Preview")

                        else:
                            st.error(
                                "❌ Failed to generate video. Please check the logs for details."
                            )
