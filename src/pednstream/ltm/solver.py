...
class NodeFlowSolver:
    """
    Stateless solver that computes node flow assignments.
    It inspects the node's type and dispatches to the appropriate algorithm.
    """
    ...
    def solve(node, s: np.ndarray, r: np.ndarray, type: str = "classic") -> np.ndarray:
        ...
        if node.node_type == "onetoone":
            return NodeFlowSolver._solve_onetoone(node, s, r)
        elif node.node_type == "regular":
            return NodeFlowSolver._solve_regular(node, s, r, type=type)
        else:
    ...
    def _solve_regular(node, s: np.ndarray, r: np.ndarray, type: str = "classic") -> np.ndarray:
        if type == "optimal":
            return NodeFlowSolver._solve_regular_optimal(node, s, r)
        elif type == "classic":
            return NodeFlowSolver._solve_regular_classic(node, s, r)
        else:
            raise ValueError(f"Invalid type: {type}")

    @staticmethod
    def _solve_regular_optimal(node, s: np.ndarray, r: np.ndarray) -> np.ndarray:
        A_eq = NodeFlowSolver._build_matrix_A_eq(
