    ...
    def __call__(self, density: float) -> float:
        ...
        if self.model_type == "greenshields":
            if density <= self.k_critical:
                ...
                v = max(
                    0,
                    -self.v_f * (density - self.k_jam) / (self.k_jam - self.k_critical),
                )
        ...
        # type2: triangular density flow fundamental diagram (Yperman's LTM)
            ...
            if density <= self.k_critical:
                ...
                v = max(
                    0,
                    (self.k_critical * self.v_f)
                    / (self.k_jam - self.k_critical)
                    * (self.k_jam / density - 1),
                )
...
def travel_cost(link, inflow, outflow):
    ...
    return link.length / (
        link.free_flow_speed * (1 - (inflow + outflow) / link.capacity)
    )
...
class BiDirectionalFd:
    def __init__(
        self, v_f, k_critical, k_jam, model_type="yperman", bi_factor=1.5, noise_std=0
    ):
        self.v_f, self.k_critical, self.k_jam, self.bi_factor = (
            v_f,
            k_critical,
            k_jam,
            bi_factor,
    ...
    def __call__(self, k_self, k_opp):
        ...
        if self.model_type == "greenshields":
            if k_eff <= self.k_critical:
                ...
                v = max(
                    0, -self.v_f * (k_eff - self.k_jam) / (self.k_jam - self.k_critical)
                )
        elif self.model_type == "yperman":
            if k_eff <= self.k_critical:
                v = self.v_f
